...
static bool check_three_flag_combinations(t_lst *head, int *i, int j, 
                                    bool *no_new_line) {
...
static bool check_e_flag_combinations(t_lst *head, int j, bool *escape_off, bool *no_new_line) {
...
int mx_echo(t_global *hd, t_lst *head) {
    ...
    if (!head->av[1])
        ...
        print_arguments(head, i, hd, escape_off);
        if (!no_new_line)
            mx_printstr("\n");
    }
    return 1;
