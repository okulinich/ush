...
// 	return 1;
// }
static void cd(t_lst *h, t_global *s) {
	if (h->av[1]) {
		chdir(h->av[1]);
		if (mx_strcmp(h->av[1], ".") != 0
            && mx_strcmp(h->av[1], "./") != 0) {
            setenv("OLDPWD", mx_get_env_var(s->env, "PWD"), 1);
...
int	mx_cd(t_global *s, t_lst *h) {
	...
	if (h->av[1] != NULL) {
		...
		chdir(mx_get_env_var(s->env, "HOME"));
		setenv("OLDPWD", mx_get_env_var(s->env, "PWD"), 1); // leaks? 
		setenv("PWD", mx_get_env_var(s->env, "HOME"), 1); // leaks? 
	}
	return 1;
}
